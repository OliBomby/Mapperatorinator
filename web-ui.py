...
process_lock = threading.Lock()  # Lock for accessing processes safely
...
def start_inference():
    ...
    try:
        ...
        print(f"Started process with PID: {process.pid} for job_id: {job_id}")
        # Return success to the AJAX call
        return jsonify({"status": "success", "message": "Inference started", "job_id": job_id}), 202  # Accepted
    ...
    def generate():
        ...
        with process_lock:
            ...
            if process and process.poll() is None:
                process_to_stream = process
                print(f"Attempting to stream output for PID: {process_to_stream.pid} (job_id: {job_id})")
            else:
                # Handle case where process is already finished or never started
                if process:
                    processes.pop(job_id, None)
                    cancelled_pids.discard(process.pid)
                print(f"Stream requested but no active process found or process already finished for job_id: {job_id}.")
        ...
        if process_to_stream:
            ...
            try:
                ...
                with process_lock:
                    processes.pop(job_id, None)
                    cancelled_pids.discard(process_to_stream.pid)
                    print(f"Cleared process reference for job_id: {job_id}.")
...
def cancel_inference():
    ...
    with process_lock:
        ...
        if process and process.poll() is None:
            try:
                pid = process.pid
                print(f"Attempting to terminate process PID: {pid} (job_id: {job_id})...")
                ...
                if sys.platform == 'win32':
                    ...
                    process.terminate()  # Send SIGTERM on Unix
                    try:
                ...
                success = True
                status_code = 200
                # DO NOT remove process here. Let the stream generator handle it.
            except Exception as e:
            ...
            message = "No process is currently running for the provided job_id."
            success = False
            status_code = 404
