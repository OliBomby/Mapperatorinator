...
class RoPEWhisperPositionalEmbedding(nn.Embedding):
...
class RoPEWhisperAttention(nn.Module):
    ...
    def __init__(
            ...
            config: Optional[RoPEWhisperConfig] = None,
    ):
        super().__init__()
    ...
    def forward(
        ...
        # query_states will be (bsz, num_heads, tgt_len, head_dim)
        query_states = self._shape(self.q_proj(hidden_states) * self.scaling, tgt_len, bsz)

        # Prepare cache access:
        # `active_cache` will point to the specific cache object (e.g., self_attention_cache or cross_attention_cache)
        # `original_past_key_value` is kept to be returned, as it's the top-level object that might have been mutated.
        original_past_key_value = past_key_value
        active_cache = past_key_value
        # `is_updated_flag` is used for cross-attention to check if KVs were cached in a *previous* call.
        is_updated_flag = False

        if past_key_value is not None:
            is_updated_flag = past_key_value.is_updated.get(self.layer_idx, False)
            if is_cross_attention:
                # Mark that cross-attention KVs (encoder outputs) are being processed/cached for this layer.
                # This flag is for subsequent generation steps to reuse these KVs.
                past_key_value.is_updated[self.layer_idx] = True
                if hasattr(past_key_value,
                           'cross_attention_cache') and past_key_value.cross_attention_cache is not None:
                    active_cache = past_key_value.cross_attention_cache
                # If no distinct sub-cache, active_cache remains the original past_key_value.
            else:  # Self-attention
                if hasattr(past_key_value, 'self_attention_cache') and past_key_value.self_attention_cache is not None:
                    active_cache = past_key_value.self_attention_cache
                # If no distinct sub-cache, active_cache remains the original past_key_value.

        # Determine the source for K, V projections
        current_kv_source_states = key_value_states if key_value_states is not None else hidden_states

        if is_cross_attention and active_cache and is_updated_flag:
            key_states = active_cache.key_cache[self.layer_idx]
            value_states = active_cache.value_cache[self.layer_idx]
        else:

            k_proj_output = self.k_proj(current_kv_source_states)

            key_s_current_segment = self._shape(k_proj_output, -1, bsz)  # -1 infers kv_seq_len from k_proj_output

            # Project and shape V (current segment)
            value_s_current_segment = self._shape(self.v_proj(current_kv_source_states), -1, bsz)

            if position_embeddings is not None:
                cos, sin = position_embeddings
                # apply_rotary_pos_emb modifies query_states and key_s_current_segment
                query_states, key_s_current_segment_after_rope = apply_rotary_pos_emb(
                    query_states, key_s_current_segment, cos, sin
                )
            else:
                # No RoPE applied to K (or Q, if pos_emb were globally None)
                key_s_current_segment_after_rope = key_s_current_segment

            if active_cache is not None:
                effective_cache_position = cache_position if not is_cross_attention else None

                key_states, value_states = active_cache.update(
                    key_s_current_segment_after_rope,  # This is the RoPE'd current K segment
                    value_s_current_segment,
                    self.layer_idx,
                    cache_kwargs={"cache_position": effective_cache_position}  # Original used {"cache_position":...}
                )
            else:
                # No cache is used. key_states and value_states are just the current (RoPE'd) K and V.
                key_states = key_s_current_segment_after_rope
                value_states = value_s_current_segment

        attn_weights = torch.matmul(query_states, key_states.transpose(2, 3))

        if attention_mask is not None:
            # Ensure attention_mask covers the correct key sequence length
            current_kv_seq_len = key_states.shape[-2]
            if attention_mask.shape[-1] > current_kv_seq_len:  # Example check, adapt as needed
                attention_mask_to_apply = attention_mask[:, :, :, :current_kv_seq_len]
            else:
                attention_mask_to_apply = attention_mask
            attn_weights = attn_weights + attention_mask_to_apply
        ...
        return attn_output, returned_attn_weights, original_past_key_value


class RoPEWhisperFlashAttention2(RoPEWhisperAttention):
    """
    ...
    def forward(
        ...
        # get query proj
        query_states = torch.reshape(self.q_proj(hidden_states), (bsz, tgt_len, self.num_heads, self.head_dim)).transpose(1, 2)
...
class RoPEWhisperSdpaAttention(RoPEWhisperAttention):
    def forward(
            self,
            hidden_states: torch.Tensor,
            key_value_states: Optional[torch.Tensor] = None,
...
WHISPER_ATTENTION_CLASSES = {
    "eager": RoPEWhisperAttention,
    "flash_attention_2": RoPEWhisperFlashAttention2,
    "sdpa": RoPEWhisperSdpaAttention,
}


# Copied from transformers.models.mbart.modeling_mbart.MBartEncoderLayer with MBart->Whisper, MBART->WHISPER
class RoPEWhisperEncoderLayer(nn.Module):
    def __init__(self, config: RoPEWhisperConfig):
        super().__init__()
...
class RoPEWhisperDecoderLayer(nn.Module):
    def __init__(self, config: RoPEWhisperConfig, layer_idx: int = None):
        super().__init__()
        self.embed_dim = config.d_model
        ...
        self.encoder_attn_layer_norm = nn.RMSNorm(self.embed_dim)
...
class RoPEWhisperPreTrainedModel(PreTrainedModel):
    config_class = RoPEWhisperConfig
    base_model_prefix = "model"
    main_input_name = "input_features"
    supports_gradient_checkpointing = True
    _no_split_modules = ["RoPEWhisperEncoderLayer", "RoPEWhisperDecoderLayer"]
...
class RoPEWhisperEncoder(RoPEWhisperPreTrainedModel):
    """
...
class RoPEWhisperDecoder(RoPEWhisperPreTrainedModel):
    """
    ...
    def __init__(self, config: RoPEWhisperConfig):
        ...
        self.layers = nn.ModuleList(
            [RoPEWhisperDecoderLayer(config, layer_idx) for layer_idx in range(config.decoder_layers)]
        )
...
class RoPEWhisperModel(RoPEWhisperPreTrainedModel):
    def __init__(self, config: RoPEWhisperConfig):
        super().__init__(config)

        self.encoder = RoPEWhisperEncoder(config)
        self.decoder = RoPEWhisperDecoder(config)
        # Initialize weights and apply final processing
        self.post_init()
...
class RoPEWhisperForConditionalGeneration(WhisperGenerationMixin, RoPEWhisperPreTrainedModel):
    base_model_prefix = "model"
    _tied_weights_keys = ["proj_out.weight"]

    def __init__(self, config: RoPEWhisperConfig):
        super().__init__(config)
        self.model = RoPEWhisperModel(config)
...
class RoPEWhisperDecoderWrapper(RoPEWhisperPreTrainedModel):
    """
...
class RoPEWhisperForCausalLM(RoPEWhisperPreTrainedModel, GenerationMixin):
    _tied_weights_keys = ["proj_out.weight"]
    main_input_name = "input_ids"
    ...
    def __init__(self, config):
        super().__init__(config)
        config.is_encoder_decoder = False
        self.model = RoPEWhisperDecoderWrapper(config)
...
class RoPEWhisperForAudioClassification(RoPEWhisperPreTrainedModel):
    def __init__(self, config):
        super().__init__(config)
